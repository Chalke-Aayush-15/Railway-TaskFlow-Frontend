...
import { Button, Modal, Input, Avatar, Badge, EmptyState, Spinner, ProgressBar } from '../components/UI';
    ...
    <AppLayout>
      <Topbar
        ...
        actions={
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {isAdmin && (
              <>
                <Button variant="ghost" size="sm" onClick={() => setAddMemberModal(true)}>
                  + Member
                </Button>
              ...
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <div style={{ fontSize: 12, color: 'var(--text3)' }}>{done}/{total} tasks done</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--primary)' }}>{pct}%</div>
                  </div>
                ...
                <div style={{ display: 'flex', flexShrink: 0 }}>
                  {members.slice(0, 5).map((m, i) => (
                    <Avatar
                      key={m.id || m._id} name={m.name} size={28}
                      style={{
                        marginLeft: i === 0 ? 0 : -10,
              ...
              <div style={{
                ...
                {tasks.length === 0 ? (
                  <EmptyState
                    icon="📭"
                    title="No tasks yet"
                    description={isAdmin ? 'Click + Task to create the first task' : 'No tasks in this project'}
                  />
                ) : (
                        ...
                        >
                          ...
                          <td style={{ padding: '14px 16px' }}>
                            <div style={{ display: 'flex', gap: 6 }}>
                              <Button variant="subtle" size="sm" onClick={() => setStatusModal(t)}>
                                Update
                              </Button>
                              {isAdmin && (
                                <Button variant="ghost" size="sm" onClick={() => setTaskModal({ task: t })}>
                                  Edit
                                </Button>
                              )}
                            </div>
                    ...
                    >
                      ...
                      {isAdmin && (
                        <button
                          ...
                          style={{
                            background: 'none', border: 'none', cursor: 'pointer',
                            color: 'var(--text3)', fontSize: 13, padding: '2px 5px',
      ...
      {addMemberModal && (
        <Modal title="Add Team Member" onClose={() => { setAddMemberModal(false); setMemberId(''); }} width={400}>
          <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 18, lineHeight: 1.65 }}>
            Ask the team member to copy their <strong style={{ color: 'var(--text)' }}>User ID</strong> from their Profile page, then paste it below.
          </div>
          <Input
            label="User ID"
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <Button variant="ghost" onClick={() => { setAddMemberModal(false); setMemberId(''); }} style={{ flex: 1 }}>
              Cancel
            </Button>
            <Button variant="primary" onClick={handleAddMember} loading={addingMember} style={{ flex: 1 }}>
              Add Member
            </Button>
          </div>
        </Modal>
